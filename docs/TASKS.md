...
## M8 — UX Feedback Round & Tasks Tab (ROADMAP R8 Phase 1)

- [x] M8.1 Source Control branch awareness: chip with current branch + `↑ahead ↓behind`, switch/create via sheet; `/api/git/branches` + `/api/git/checkout` — branch names validated (`check-ref-format`, leading-dash rejected), plain non-forced checkout (git refuses rather than clobbers), "Stash first" affordance when the tree is dirty
- [x] M8.2 Tap-vs-scroll guard (`tap-button.tsx`): terminal shortcut keys + floating toolbar fire on `pointerup` within a 10px slop (was `pointerdown`) — scrolling the key strip no longer presses keys; keyboard-focus retention preserved
- [x] M8.3 Plugin skills viewable: `assertSkillPath` gained read/write modes — enabled Claude plugins' skill dirs are readable (listed read-only: no delete, view-only editor) while write/delete stays confined to user-owned dirs; error banners wrap long paths (`wrap-anywhere`)
- [x] M8.4 Terminal readability: re-measure + refit + PTY resize after the mono webfont loads (fixes wrong-column hard-wrap fragments in scrollback), `A− / A+` font size (9–18px, persisted, PTY reflows to the new column count), jump-to-bottom button while reading scrollback
- [x] M8.5 Terminal touch = read/scroll only: `touchend` prevented so taps never focus xterm's hidden textarea (no keyboard popup mid-scroll); mobile input goes through the prompt bar + shortcut keys; desktop mouse/hardware keyboards unaffected
- [x] M8.6 Tasks tab (bottom nav) with subtabs — **Jira** (only when `JIRA_BASE_URL`/`JIRA_EMAIL`/`JIRA_API_TOKEN` are set; token proxied server-side, never sent to the browser): assigned-issue list + full detail (status, epic, labels, priority, description converted ADF→markdown with links/code/mentions, tolerant of unknown nodes); **My Tasks**: markdown files in `~/.beam/tasks` (create/edit/delete, path-guarded API)
- [x] M8.7 Run-task flow (both kinds): auto-composed editable prompt → installed agent (default Claude Code) → directory (**required**) → optional multi-select skills/subagents (appended to the prompt by name) → new PTY session; the server types the prompt into the agent's TUI after boot (`initialInput`: waits for output to go quiet, multi-line wrapped in bracketed paste)
- [x] M8.8 Verification: 37 in-process functional checks (branch ops incl. conflict-refusal & detached HEAD, skill-guard modes, ADF converter, task-storage guard, real-PTY prompt injection) + HTTP smoke against a mock Jira server; `tsc`/ESLint/`next build` clean
- [ ] M8.9 (Manual) On-device QA: touch scroll without keyboard popup, font resize + TUI reflow, run a Jira task end-to-end with real credentials (prompt lands in Claude's TUI intact)
...
| 2026-07-13 | **UX feedback round** (M8.1–M8.5): branch switch/create in Source Control (`/api/git/branches` + `/api/git/checkout`, non-forced, "Stash first" when dirty); tap-vs-scroll guard for shortcut keys & floating toolbar (`tap-button.tsx`); plugin-shipped Claude skills viewable read-only (`assertSkillPath` read/write modes); terminal readability — refit after webfont load (fixes broken hard-wrap fragments), `A− / A+` font size with PTY reflow, jump-to-bottom; terminal touch made read/scroll-only (no keyboard popup while scrolling)                                                                            |
| 2026-07-14 | **Tasks tab — ROADMAP R8 Phase 1** (M8.6–M8.8): Jira subtab (env-gated `JIRA_*`, token proxied server-side; assigned issues, full detail with ADF→markdown description incl. clickable links/code, epic/labels/priority) + My Tasks subtab (markdown files in `~/.beam/tasks`); unified run flow — editable auto-composed prompt, installed agent, required directory, optional skills/subagents, prompt auto-typed into the agent TUI via `initialInput` (quiet-detection + bracketed paste). Verified with 37 functional checks + mock-Jira HTTP smoke; README/env docs updated, ROADMAP R8 added                       |
